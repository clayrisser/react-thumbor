...
var Thumbor = (function (_Component) {
  ...
  _createClass(Thumbor, [{
    ...
    value: function smartlyGuessDimensions() {
      ...
      if ((!width || width.substring(width.length - 1) === '%' || width.substring(width.length - 1) === 'vh' || width.substring(width.length - 1) === 'vw') && (!height || height.substring(height.length - 1) === '%' || height.substring(height.length - 1) === 'vh' || height.substring(height.length - 1) === 'vw') && !realWidth && !realHeight) {
        var guessed = false;
        if (maxWidth) {
